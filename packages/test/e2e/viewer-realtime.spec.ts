...
async function startHello(page: Parameters<typeof openViewer>[0]) {
	...
	await runCell(page, { suite: "smoke", scenario: "hello", host: "cursor" }).click();
}
...
test("viewer live updates › replays a direct agent stream after a browser reload", async ({
	page,
}) => {
...
test("viewer live updates › shows recovery, then resumes a second viewer without stale or duplicate activity", async ({
	page,
	context,
}) => {
	viewer = await openViewer(page, {
	...
	await expect(observer.locator("#live-connection")).toHaveAttribute("data-state", "reconnecting");
