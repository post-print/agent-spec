...
const BOOTSTRAP_SCRIPT = /<script type="application\/json" id="bootstrap-data">(.*?)<\/script>/s;
...
function embeddedBootstrap(html: string): ViewerBootstrap {
	const json = html.match(BOOTSTRAP_SCRIPT)?.[1];
	if (!json) throw new Error("bootstrap-data was not embedded");
	return JSON.parse(json) as ViewerBootstrap;
}

const comparisonFailureFixture: Parameters<typeof formatFailureDebugCopy>[0] = {
	suite: "tour",
...
it("viewer page › copies comparison measurements, gate values, and agent trace for debugging", () => {
	const copied = formatFailureDebugCopy(comparisonFailureFixture);
...
it("viewer page › renders a React shell with one escaped catalog and bootstrap payload", () => {
...
it("viewer page › preserves run and comparison data for the React client", () => {
