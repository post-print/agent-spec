...
it("describeRubricChecks › lists each smoke check in one line", () => {
	expect(
		describeRubricChecks({
			must: ["smoke ok"],
...
it("describeRubricChecks › distinguishes a successful command requirement from invocation", () => {
	expect(describeRubricChecks({ mustRunSuccessfully: ["bun test"] })).toEqual([
		'run "bun test" successfully',
	]);
});

it("describeTraceHappened › summarizes a no-tool reply", () => {
	expect(
		describeTraceHappened({
...
it("describeTraceHappened › names a Read path", () => {
	expect(
		describeTraceHappened({
...
it("displayToolPath › strips the sealed workspace prefix", () => {
...
it("buildScenarioStory › marks a pass with criteria and result", () => {
...
it("buildScenarioStory › lists each compare arm description in the criteria", () => {
	const story = buildScenarioStory({
		rubric: {},
		passed: true,
		failures: [],
	...
	expect(story.criteria).toContain("control: Intact catalog. Names the real Billing API webhook.");
	expect(story.criteria).toContain("experimental: Contested catalog. Reports duplicate summaries.");
});

it("buildScenarioStory › lists both compare arms in the result", () => {
	const story = buildScenarioStory({
...
it("buildScenarioStory › lists comparison gates without a judge", () => {
	const story = buildScenarioStory({
		rubric: {},
		passed: true,
		failures: [],
...
it("buildScenarioStory › lists named arms and pair gates in the criteria", () => {
	const story = buildScenarioStory({
		rubric: {},
		passed: true,
		failures: [],
...
it("buildScenarioStory › lists per-arm rubric checks on a skill compare", () => {
	const story = buildScenarioStory({
...
it("buildScenarioStory › puts a worktree leak in the result verdict", () => {
	const story = buildScenarioStory({
...
it("buildScenarioStory › scores shared rubric checks on each compare arm", () => {
	const story = buildScenarioStory({
...
it("buildScenarioStory › scores a failed comparison gate on the compare section", () => {
	const story = buildScenarioStory({
		rubric: {},
		passed: false,
		failures: [
...
it("buildScenarioStory › shows an expected failed arm check as failed", () => {
	const story = buildScenarioStory({
		rubric: {},
		passed: true,
		failures: [],
...
it("buildScenarioStory › keeps an unmatched worktree leak on the leftover verdict", () => {
	const story = buildScenarioStory({
