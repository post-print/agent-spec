...
it("parseClaudeNdjsonLine › parses valid JSON lines and skips blanks/invalid", () => {
...
it("normalizeClaudeUsage › maps snake_case Anthropic usage fields", () => {
	expect(
		normalizeClaudeUsage({
			input_tokens: 10,
			output_tokens: 5,
...
it("normalizeClaudeToolArgs › exposes command for Bash tools", () => {
...
it("buildTraceFromClaudeEvents › maps assistant text, Bash tool_use/result, and usage", () => {
...
it("buildTraceFromClaudeEvents › records a failed Bash tool result without inventing an exit code", () => {
	const failedResult: ClaudeStreamEvent = {
		type: "user",
		message: {
			role: "user",
			content: [{ type: "tool_result", tool_use_id: "toolu_1", content: "failed", is_error: true }],
		},
	};
...
it("buildTraceFromClaudeEvents › records AskUserQuestion tool calls for fail-fast detection", () => {
...
it("buildTraceFromClaudeEvents › coalesces stream_event text deltas", () => {
...
it("isUserInputTool Claude names › matches AskUserQuestion variants", () => {
