...
it("capture › keeps a raw node command for mustRun", () => {
	const cmds = extractShellCommandsFromToolCalls([
		{
			name: "shell",
			args: {
...
it("capture › extracts invoked skills from Read tool paths", () => {
...
it("capture › builds skillsInvoked on SDK traces", () => {
	const events = [
		{
			type: "assistant",
			message: {
...
it("capture › resolves a total from input and output when totalTokens is missing", () => {
...
it("capture › accumulates usage events into trace.usage", () => {
...
it("capture › captures MCP tool calls with args and result from SDK root result", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "tool_call",
			call_id: "call-echo-1",
...
it("capture › captures shell commands from Cursor SDK tool_call events", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "tool_call",
			name: "shell",
...
it("capture › captures Cursor shell success from the structured result", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "tool_call",
			call_id: "shell-1",
...
it("capture › does not treat skill-name prose as an invoke", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "assistant",
			message: {
...
it("capture › infers review depth from synthesis header", () => {
	expect(inferReviewDepthFromText("Review · staged · Standard · path")).toBe("standard");
});

it("capture › extracts shell commands from tool output", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "assistant",
			message: {
...
it("capture › enriches tier from streamed token chunks without newline joins", () => {
	const enriched = enrichTrace({
		messages: [
...
it("capture › finalizes streamed assistant chunks into a contiguous tier match", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "assistant",
			message: {
...
it("capture › coalesces streamed assistant tokens and flushes a new turn after tools", () => {
	const trace = buildTraceFromSdkMessages([
		{
			type: "assistant",
			message: {
...
it("capture › requires tier in assistant prefix before tool calls", () => {
...
it("capture › uses the full assistant reply when no tool was called", () => {
...
it("capture › requires ## Routing in assistant prefix before tool calls", () => {
	const pass: AgentTrace = {
		messages: [
			{
				role: "assistant",
