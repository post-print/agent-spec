...
The default test config selects OpenAI; the separate matrix config has OpenAI, Claude, and Cursor projects. Live tests require the selected host's authentication; the tour's explicit OpenAI reviewer requires its own authentication. No provider-backed tests run in CI.
